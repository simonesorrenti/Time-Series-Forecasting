...
def build_model(input_shape, output_shape)
    ...
    lstm_1 = tfkl.Bidirectional(tfkl.LSTM(32, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5)))(batch)

    lstm_main_1 = tfkl.Bidirectional(tfkl.LSTM(32, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
    ...
    lstm_main_2 = tfkl.Bidirectional(tfkl.LSTM(64, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5)))(skip) # main2
    lstm_2 = tfkl.Bidirectional(tfkl.LSTM(64, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
    ...
    lstm_main_out = tfkl.Bidirectional(tfkl.LSTM(64, return_sequences=True, dropout=0.1, recurrent_dropout=0.1, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5)))(skip) # main2

    mlp = tfkl.Dense(2048, activation='relu', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(batch)
    mlp = tfkl.BatchNormalization()(mlp) 
    mlp = tfkl.Dropout(0.1)(mlp)
    mlp = tfkl.Dense(1024, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(mlp)
    mlp = tfkl.BatchNormalization()(mlp) 
    mlp = tfkl.Dropout(0.1)(mlp)
    mlp = tfkl.Dense(512, activation='relu', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(mlp)
    
    dense = tf.keras.layers.Concatenate()([lstm_main_out, input_layer, mlp])
    dense = tfkl.BatchNormalization()(dense) 
    
    dense = tfkl.Dense(2048, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(dense)
    dense = tfkl.BatchNormalization()(dense) 
    dense = tfkl.Dropout(0.1)(dense)
    dense = tfkl.Dense(1024, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(dense)
    dense = tfkl.BatchNormalization()(dense) 
    dense = tfkl.Dropout(0.1)(dense)
    dense = tfkl.Dense(7, activation='linear', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(dense)
