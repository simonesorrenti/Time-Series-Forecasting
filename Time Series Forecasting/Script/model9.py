...
def build_model(input_shape, output_shape):
    ...
    lstm_1 = tfkl.Bidirectional(
        tfkl.LSTM(75, return_sequences=True, dropout=0.2, recurrent_dropout=0.2, activation='tanh',
    ...
    lstm = tfkl.Bidirectional(
        tfkl.LSTM(200, return_sequences=False, dropout=0.2, recurrent_dropout=0.2, activation='tanh',
                  kernel_initializer=tfk.initializers.GlorotUniform(seed=seed),
                  kernel_regularizer=tf.keras.regularizers.l2(1e-5)))(skip)
    ...
    dense = tfkl.Dense(output_shape[-2] * output_shape[-1] * 2, activation='tanh',
                       kernel_initializer=tfk.initializers.GlorotUniform(seed=seed),
                       kernel_regularizer=tf.keras.regularizers.l2(1e-5))(batch)
    dense = tfkl.BatchNormalization()(dense)
    dense = tfkl.Dropout(0.2)(dense)
    dense = tfkl.Dense(output_shape[-2] * output_shape[-1], activation='linear',
                       kernel_initializer=tfk.initializers.GlorotUniform(seed=seed),
    ...
    model.compile(loss=tfk.losses.MeanSquaredError(), optimizer=optimizer,
                  metrics=['mae', rmse, get_lr_metric(optimizer)])
...
for i in range(len(window)):
    ...
    history = model.fit(
        ...
        callbacks=[tfk.callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=15, restore_best_weights=True),
                   tfk.callbacks.ReduceLROnPlateau(monitor='val_loss', mode='min', factor=0.5, patience=4,
                                                   min_lr=1e-15)]
    ).history
