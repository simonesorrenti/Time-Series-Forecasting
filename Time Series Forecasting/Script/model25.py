...
def build_CONV_LSTM_model(input_shape, output_shape):
    ...
    convlstm1 = tfkl.Conv1D(64, 6, padding='same', activation='relu')(convlstm)
    convlstm2 = tfkl.Conv1D(64, 12, padding='same', activation='relu')(convlstm)
    convlstm3 = tfkl.Conv1D(64, 18, padding='same', activation='relu')(convlstm)
    concatted = tf.keras.layers.Concatenate()([convlstm1, convlstm2, convlstm3])
    convlstm = tfkl.MaxPool1D()(concatted)
    convlstm = tfkl.BatchNormalization()(convlstm)
    convlstm1 = tfkl.Conv1D(128, 6, padding='same', activation='relu')(convlstm)
    convlstm2 = tfkl.Conv1D(128, 12, padding='same', activation='relu')(convlstm)
    convlstm3 = tfkl.Conv1D(128, 18, padding='same', activation='relu')(convlstm)
    concatted = tf.keras.layers.Concatenate()([convlstm1, convlstm2, convlstm3])
    convlstm = tfkl.MaxPool1D()(concatted)
    convlstm = tfkl.BatchNormalization()(convlstm)
    convlstm1 = tfkl.Conv1D(256, 6, padding='same', activation='relu')(convlstm)
    convlstm2 = tfkl.Conv1D(256, 12, padding='same', activation='relu')(convlstm)
    convlstm3 = tfkl.Conv1D(256, 18, padding='same', activation='relu')(convlstm)
    concatted = tf.keras.layers.Concatenate()([convlstm1, convlstm2, convlstm3])
    convlstm = tfkl.MaxPool1D()(concatted)
    convlstm = tfkl.BatchNormalization()(convlstm)
    convlstm = tfkl.Bidirectional(tfkl.LSTM(1024, return_sequences=True, dropout=0.2))(convlstm)
    convlstm = tfkl.Bidirectional(tfkl.LSTM(1024, return_sequences=True, dropout=0.2))(convlstm)
    convlstm = tfkl.Bidirectional(tfkl.LSTM(1024, return_sequences=True, dropout=0.2))(convlstm)
    convlstm = tfkl.Bidirectional(tfkl.LSTM(1024, return_sequences=True, dropout=0.2))(convlstm)
    convlstm1 = tfkl.Conv1D(1024, 6, padding='same', activation='relu')(convlstm)
    convlstm2 = tfkl.Conv1D(1024, 12, padding='same', activation='relu')(convlstm)
    convlstm3 = tfkl.Conv1D(1024, 18, padding='same', activation='relu')(convlstm)
    concatted = tf.keras.layers.Concatenate()([convlstm1, convlstm2, convlstm3])
