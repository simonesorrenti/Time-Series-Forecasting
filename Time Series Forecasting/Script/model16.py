...
def build_model(input_shape, output_shape):
    ...
    dense = tfkl.Dense(output_shape[-2]*output_shape[-1]*3, activation='tanh', kernel_initializer=tfk.initializers.GlorotUniform(seed=seed), 
                               kernel_regularizer=tf.keras.regularizers.l2(1e-5))(batch)
    dense = tfkl.Dropout(0.2, seed=seed)(dense)
...
for i in range(len(window)):
    ...
    X_train = X[0:len(X)-10]
    X_val = X[len(X)-10:]
    y_train = y[0:len(X)-10]
    y_val = y[len(X)-10:]
