...
#window = [864, 2592, 4320, 6048]
window = [2592, 4320, 6048]
#stride = [6, 12, 18, 24]
stride = [12]
telescope = 864
#batch = [64, 128, 256]
...
def build_CONV_LSTM_model(input_shape, output_shape):
    ...
    convlstm = tfkl.Conv1D(32, 3, padding='same', activation='relu')(convlstm)
    ...
    dense = tfkl.Dropout(.3)(convlstm)
    dense = tfkl.Dense(output_shape[-1]*output_shape[-2]*2, activation='relu')(dense)
    dense = tfkl.Dropout(.3)(dense)
    ...
    for s in stride:
        for b in batch:
            for sp in split:
                ...
                model.save('CNN-BiLSTM_window'+str(w)+'_stride'+str(s)+'_batch'+str(b)+'_split'+str(sp)+'_Model4.h5')
                ...
                plt.title('Mean Squared Error window'+str(w)+'_stride'+str(s)+'_batch'+str(b)+'_split'+str(sp)+'_Model4')
                plt.legend()
                plt.grid(alpha=.3)
                plt.show()
                ...
                plt.axvline(x=best_epoch, label='Best epoch', alpha=.3, ls='--', color='#5a9aa5')
                plt.title('Mean Absolute Error window'+str(w)+'_stride'+str(s)+'_batch'+str(b)+'_split'+str(sp)+'_Model4')
                plt.legend()
                plt.grid(alpha=.3)
                plt.show()
